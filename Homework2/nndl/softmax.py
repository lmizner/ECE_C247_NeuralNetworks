...
class Softmax(object):
  ...
  def loss(self, X, y):
    ...
    for i in range(scores.shape[1]):
        score = scores[:, i]
        score -= np.max(score)
        current_class_score = score[y[i]]
        loss += np.log(np.sum(np.exp(score)))
        loss -= current_class_score
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    corres_probs = probs[y, range(number_train)]
    log_probs = -np.log(corres_probs.clip(min=np.finfo(float).eps))
    
    loss = np.sum(log_probs) / number_train

    probs[y, range(number_train)] -= 1
    grad = np.dot(probs, X)
    grad /= number_train
