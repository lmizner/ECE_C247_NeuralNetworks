...
def batchnorm_backward(dout, cache):
  ...
  # Retrieve paramters from cache
  N = dout.shape[0]
  mb_var = cache.get('mb_var')
  x_center = cache.get('x_center')
  ...
  # Calculate dx
  dx_hat = dout * gamma
  dx_mu1 = dx_hat / np.sqrt(mb_var + eps)

  sqrt_var = np.sqrt(mb_var + eps)
  dsqrt_var = -np.sum(dx_hat * x_center, axis = 0) / (sqrt_var**2)
  dvar = dsqrt_var * 0.5 / sqrt_var

  dx_mu2 = 2 * x_center * dvar * np.ones_like(dout) / N
