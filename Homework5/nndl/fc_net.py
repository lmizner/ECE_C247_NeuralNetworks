...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None):
    ...
    sum_term = np.sum(W1**2) + np.sum(W2**2)
    reg_term = 0.5 * self.reg * sum_term
    loss += reg_term
      
    d_hidden, dw2, db2 = affine_backward(d_scores, scores_cache)
    dx, dw1, db1 = affine_relu_backward(d_hidden, hidden_cache)
...
class FullyConnectedNet(object):
  ...
  def loss(self, X, y=None):
    ...
    for i in range(self.num_layers - 1):
      ...
      if self.use_batchnorm:
        batchnorm_out, batchnorm_cache[str(i + 1)] = batchnorm_forward(fc_out, self.params['gamma' + str(i + 1)], 
